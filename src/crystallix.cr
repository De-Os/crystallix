  ...
  module CRYSTALLIX
    ...
    def self.generateServer
      HTTP::Server.new do |context|
        ...
        if Dir.exists?(file)
          if file[-1, 1] != "/"
            headers["Host"] = "http://" + headers["Host"] if !headers["Host"].starts_with?("http")
            context.response.headers["Location"] = headers["Host"] + resource + "/"
        ...
        if context.response.status_code != 308
          if File.exists?(file)
          ...
          context.response.headers["Content-Type"] = MIME.from_filename(file) + "; charset=UTF-8" if !context.response.headers["Content-Type"]? && File.exists?(file) && MIME.from_filename?(file)
        end
